...
st.title("📦 Tesco 발주 데이터 자동 변환기 (다중 주문서 병합)")
st.write("발주 시스템 원본 파일 하나만 올리시면, **파일 내에 분할된 모든 표를 하나로 긁어모아** 누락 없이 7개 열로 추출합니다.")
...
if raw_file:
    try:
        with st.spinner("모든 주문서를 하나도 빠짐없이 긁어모으는 중입니다..."):
            ...
            # --- 1. [핵심] 여러 개로 쪼개진 표(Table) 완벽 병합 로직 ---
            if raw_file.name.endswith('.csv'):
                ...
                try:
                    ...
                    # 가짜 엑셀(HTML)인 경우 파일 내의 "모든 표"를 찾아서 하나로 합침!!
                    raw_file.seek(0)
            ...
            # 컬럼 중복 제거 및 중간에 낀 불필요한 제목 행들 제거
            ...
            def get_store_info(row):
                ...
                if key in SMART_STORE_MAP: return SMART_STORE_MAP[key]
                elif 'MIX' in type_str: return SMART_STORE_MAP.get(store_str + 'SORTATION', {'발주코드': 81021000, '배송코드': 81040913})
                else: return {'발주코드': 81021000, '배송코드': 81040913}
            ...
            st.success(f"✅ 파일 내에 숨어있던 **{len(df_final)}개**의 모든 품목/배송처 데이터가 누락 없이 완벽히 추출되었습니다!")
